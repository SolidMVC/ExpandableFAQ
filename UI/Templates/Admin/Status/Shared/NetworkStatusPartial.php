...
<form name="status_form" action="<?=esc_url($statusTabFormAction);?>" method="POST" class="status-form">
    <div style="padding-bottom: 20px;" class="big-text">
        <strong><?=esc_html($lang['LANG_STATUS_NETWORK_ENABLED_TEXT']);?>:</strong> <?=esc_html($lang['LANG_YES_TEXT']);?><br />
        <br />
        <strong><?=esc_html($lang['LANG_STATUS_DATABASE_MIN_VERSION_TEXT']);?>:</strong> <?=esc_html($minDatabaseSemver);?><br />
        <br />
    ...
    <?php if($databaseMatchesCodeSemver === FALSE): ?>
        ...
        <?php else: ?>
            <?=esc_html($lang['LANG_STATUS_UPDATE_NOT_ALLOWED_ERROR_TEXT']);?><br />
            <br />
            <?=esc_html($lang['LANG_STATUS_NETWORK_NOTE_FOR_POSSIBLY_NOT_IMPORTED_DATA_TEXT']);?>
        <?php endif; ?>
    <?php endif; ?>
</form>